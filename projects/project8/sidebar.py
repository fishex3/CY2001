...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        var["study_end"]    = st.date_input("Study window end",    value=date(2026, 1, 1), min_value=date(2026,1,1))
        ...
        var["lag_features"] = st.selectbox(
            "Feature-Target Alignment",
            options=[True, False],
            format_func=lambda x: "Forecast Mode (Lagged Features)" if x else "Nowcast Mode (Contemporaneous)",
            help="Forecast: Features lagged by 1Q predict future target. Nowcast: Features contemporary with target.",
            index=0  # Default to True (Forecast mode)
        )
        ...
        var["kappa2"]       = st.slider("Forgetting factor κ₂", 0.90, 0.999, 0.96, 0.001, format="%.3f")
        ...
        with st.expander("XGBoost"):
            var["xgb_n_est"]  = st.slider("n_estimators",  50, 800, 75, 25)
            var["xgb_depth"]  = st.slider("max_depth",       1,   8,   4)
            var["xgb_lr"]     = st.slider("learning_rate", 0.01, 0.30, 0.05, 0.01)
            var["xgb_sub"]    = st.slider("subsample",      0.5, 1.0, 0.7, 0.05)
            var["xgb_col"]    = st.slider("colsample_bytree", 0.5, 1.0, 0.7, 0.05)
            var["xgb_alpha"]  = st.slider("reg_alpha",      0.0, 2.0, 0.1, 0.1)
            var["xgb_lambda"] = st.slider("reg_lambda",     0.0, 5.0, 2.0, 0.5)
            var["xgb_mcw"]    = st.slider("min_child_weight", 1, 20, 3)
